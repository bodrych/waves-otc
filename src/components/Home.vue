...
<script>
  ...
  export default {
    computed: {
      ...mapGetters(['getOrders', 'getAddress', 'getBalance']),
      verifyOrder() {
      ...
      items() {
        const items = [];
        _.forEach(this.getOrders, (order) => {
    ...
    methods: {
      ...mapActions(['fetchOrders', 'cancelOrder', 'fetchBalance', 'makeVSTSellOrder', 'takeVSTSellOrder', 'makeVSTBuyOrder', 'takeVSTBuyOrder']),
      hideDialog() {
